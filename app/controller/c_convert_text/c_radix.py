...
def convert_from_bin(to_type: str, data_list: List[bin]):
    ...
    for data in data_list:
        if to_type == "dec":
            ...
            result_list.append(oct(data, 2))
        elif to_type == "hex":
            result_list.append(hex(data, 2))
...
def convert_from_oct(to_type: str, data_list: List[oct]):
    ...
    result_list: List[str] = list()
    for data in dec_list:
        if to_type == "bin":
            result_list.append(bin(data, 8))
        elif to_type == "dec":
            result_list.append(int(data, 8))
        elif to_type == "hex":
            result_list.append(hex(data, 8))
...
def convert_from_hex(to_type: str, data_list: List[hex]):
    ...
    for data in dec_list:
        if to_type == "bin":
            result_list.append(bin(data, 16))
        elif to_type == "oct":
            result_list.append(oct(data, 16))
        elif to_type == "dec":
            result_list.append(hex(data, 16))
